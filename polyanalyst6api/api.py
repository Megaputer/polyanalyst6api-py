...
class API:
    ...
    def get_project_export_status(self, export_id: str) -> Dict:
        ...
        .. versionadded:: 0.27.0
        """
        return self.get('project/export/status', params={'exportId': export_id})

    def export_project(
            self,
            file_path: str,
            project_ids: List[str],
            withND: bool, # by default at the server: True
            multi: bool, # by default at the server: False
            compressionLevel: int, # by default at the server: the Pa7z::Params::Normal corresponds to the compressionLevel = 5
            keepBackups: bool, # by default at the server: True
            usePAGridFormat: bool,  # by default at the server: False
            keepMacrosAndVars: bool, # by default at the server: True
            keepSliceStatistics: bool,  # by default at the server: False
            overwriteExisting: bool, # by default at the server: True
            dicts: Optional[dict] = {},
            usedDicts: Optional[dict] = {},
            usedParsers: Optional[list] = [],  # the usedParsers must be a set but presented as a list
            wait: bool = False
    ) -> Union[str, Dict]:
        """
        Export a project from the PAserver.

        :param file_path: full path to the exported file on the PA6 Drive, for example: "@administrator@/example.pa6"
        :param project_ids: id of projects to export
        :param withND: include the source data              # the parameter will be depreceated soon
        :param multi: use *.paar6 format                    # the parameter will be depreceated soon
        :param compressionLevel: select compression level (Store = 0, Fastest = 1, Fast = 3, Normal = 5, Maximum = 7, Ultra = 9)
        :param keepBackups: keep backup versions
        :param usePAGridFormat: make the exported project file compatible with PaGrid
        :param keepMacrosAndVars: keep user and server macros and variables
        :param keepSliceStatistics: keep slice usage statistics
        :param overwriteExisting: allow overwriting an existing file
        :param dicts: include a descriptions of the project dictionaries
        :param usedDicts: include descriptions of server-side non-embedded dictionaries
        :param usedParsers: include the parsers used
        ...
        resp, _ = self.request(
            ...
            json={
                ...
                "usePAGridFormat": usePAGridFormat,
                "keepMacrosAndVars": keepMacrosAndVars,
                "keepSliceStatistics": keepSliceStatistics,
        ...
        while True:
            time.sleep(1)
            status = self.get_project_export_status(export_id)
